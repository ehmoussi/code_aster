...
class A_CLASSER:
    ...
    def __init__(self, *args):
        if len(args) > 2:
            print(convert(
                ufmt(_("Erreur à la création de la règle A_CLASSER(%s)"),
                     args)))
            return
        self.args = args
        if type(args[0]) == tuple:
            ...
            print(convert(ufmt(_("Le premier argument de : %s doit etre un "
                                 "tuple ou une chaine"), args)))
        if type(args[1]) == tuple:
            self.args1 = args[1]
        elif type(args[1]) == str:
            self.args1 = (args[1],)
        else:
            print(convert(ufmt(_("Le deuxième argument de :%s doit etre un "
                                 "tuple ou une chaine"), args)))
    ...
    def verif(self, args):
        ...
        # élément de self.mcs
        if len(l_couples) == 0:
            message = ufmt(_("- Il faut qu'au moins un objet de la liste : %r"
                             " soit suivi d'au moins un objet de la liste : %r"),
        ...
        for couple in l_couples:
            ...
            if len(couple) == 1:
                ...
                if couple[0] not in self.args1:
                    text = text + ufmt(
                        _("- L'objet : %s doit être suivi d'un objet de la liste : %r\n"),
                        couple[0], self.args1)
                    test = 0
                else:
                    if num > 1:
                        ...
                        text = text + ufmt(
                            _("- L'objet : %s doit être précédé d'un objet de la liste : %r\n"),
                            couple[0], self.args0)
                        test = 0
            elif couple not in self.liste_couples:
                text = text + ufmt(
                    _("- L'objet : %s ne peut être suivi de : %s\n"),
                    couple[0], couple[1])
                test = 0
        return text, test
