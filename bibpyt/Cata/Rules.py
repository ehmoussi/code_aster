...
class AtLeastOne(Rule):
    ...
    def check(self, dictSyntax):
        ...
        if not present:
            raise NameError(
                "Syntax error - At least one in " + str(self.ruleList))
...
class OneIn(Rule):
    ...
    def check(self, dictSyntax):
        ...
        if count != 1:
            raise NameError(
                "Syntax error - Must have just one in " + str(self.ruleList))
        ...
        if present:
            for value in self.ruleList:
                if not dictSyntax.has_key(value):
                    raise NameError(
                        "Syntax error - Must have all in " + str(self.ruleList))
    ...
    def check(self, dictSyntax):
        ...
        if present:
            for value in self.ruleList[1:]:
                if dictSyntax.has_key(value):
                    raise NameError(
                        "Syntax error - " + self.ruleList[0] + " forbiden with " + value)
