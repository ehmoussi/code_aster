...
! In  vect_name        : name of elementary vector
! In  vect_type        : type (R or C) of elementary vector
...
! 
    if (mode_type .eq. 'R') then
          call jeveuo(vect_redu, 'E', vr = vr_vect_redu)
          ...
          call jeveuo(vect_redu, 'E', vc = vc_vect_redu)
          if (vect_type .eq. 'R') then
              call jeveuo(vect_name(1:8)//'           .VALE', 'L', vr=jv_vect_r)
              do i_equa = 1, nb_equa
