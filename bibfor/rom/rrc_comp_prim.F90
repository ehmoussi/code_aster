...
!
    nume_store = 0
    call romResultSetZero(resultDomName, nume_store, mode)
!
! - Reduced coordinates: Gappy POD if necessary
!
    if (l_corr_ef) then
        call romEvalGappyPOD(cmdPara, resultRomName, nbStore, v_prim_rom,&
                             v_cohr , 0)
    else
...
! ----- Set field
        ...
        enddo
! ----- Get parameters
        call rslesd(resultRomName , nume_store-1,&
                    materi_    = materi    ,&
                    cara_elem_ = cara_elem ,&
                    list_load_ = list_load )
        call rsadpa(resultRomName, 'L', 1, 'INST', nume_store, 0, sjv=jv_para)
        time = zr(jv_para)
! ----- Save parameters
        call rssepa(resultDomName, nume_store, modelDom, materi, cara_elem,&
                    list_load)
        call rsadpa(resultDomName, 'E', 1, 'INST', nume_store, 0, sjv=jv_para)
        zr(jv_para) = time
        call rsnoch(resultDomName, mode%fieldName, nume_store)
    end do
!
! - Clean
!
