...
! --------------------------------------------------------------------
!
subroutine lcRestoreStrain(option, typmod,&
                             neps , epsth , depsth,&
                             epsm , deps)
!
implicit none
...
! Behaviour
!
! Restore total strains (add "thermic" strains to mechanical strains)
!
! --------------------------------------------------------------------------------------------------
!
...
! Inout  epsm          : In : mechanical strains at beginning of current step time
!                        Out : total strains at beginning of current step time
! Inout  deps          : In : increment of mechanical strains during current step time
!                        Out : increment of total strains during current step time
...
! --------------------------------------------------------------------------------------------------
!
    call r8inir(neps, 0.d0, depstot, 1)
    call r8inir(neps, 0.d0, epsmtot, 1)
!
...
!
    if ((neps .eq. 6) .or. (neps .eq. 4)) then
        ...
        call daxpy(neps, 1.d0, depsth, 1, depstot,1)
        call dcopy(neps, epsm, 1, epsmtot, 1)
        call daxpy(neps, 1.d0, epsth, 1, epsmtot, 1)
