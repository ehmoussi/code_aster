...
void FiniteElementDescriptorInstance::transferDofDescriptorFrom(FiniteElementDescriptorPtr& other){
    if (!getSupportMesh()->isPartial())
        throw std::runtime_error( "the mesh associated to finiteElementDescriptorInstance is not a partial mesh" );
    const PartialMeshPtr partialMesh = boost::static_pointer_cast<PartialMeshInstance>(getSupportMesh());
    if ( partialMesh->getParallelMesh()!=other->getSupportMesh() )
        throw std::runtime_error( "parallel mesh associated to partial mesh of FiniteElementDescriptorInstance \n"
