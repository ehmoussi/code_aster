...
ParallelFiniteElementDescriptorInstance::ParallelFiniteElementDescriptorInstance(
    const std::string &name, const FiniteElementDescriptorPtr &FEDesc, const PartialMeshPtr &mesh,
    const ModelPtr &model, const JeveuxMemory memType )
    : FiniteElementDescriptorInstance( name, model->getSupportMesh(), memType ),
      _BaseFEDesc( FEDesc ), _joins( JeveuxVectorLong( getName() + ".DOMJ" ) ),
    ...
    if ( nbDelayedNodes > 0 ) {
        ...
        *( _joins ) = joins;
        ...
        _listOfGroupOfElements->allocateContiguous( memType, nbCollObj, totalCollSize + nbCollObj,
                                                    Numbered );
        posInCollection = 1;
