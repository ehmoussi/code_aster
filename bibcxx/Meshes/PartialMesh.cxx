...
PartialMeshInstance::PartialMeshInstance( const std::string &name, const ParallelMeshPtr &mesh,
                                          const VectorString &toFind )
    : BaseMeshInstance( name, "MAILLAGE_PARTIEL" ), _pMesh( mesh ),
    ...
    for ( int proc = 0; proc < nbProcs; ++proc ) {
        ...
        if ( proc == rank ) {
            ...
            completeMatchingNumbering.insert( completeMatchingNumbering.end(), buffer2.begin(),
                                              buffer2.end() );
        }
