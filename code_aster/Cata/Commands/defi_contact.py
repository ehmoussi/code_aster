...
# --- Method for friction
                                          ...
                                          b_frot_pena=BLOC(condition = """equal_to("ALGO_FROT", 'PENALISATION')  """, fr=tr("Paramètres frottement de la méthode pénalisée"),
                                                           ADAPTATION      =SIMP(statut='f',typ='TXM',defaut="ADAPT_COEF", into=("ADAPT_COEF","CYCLAGE","TOUT","NON"),),
                                                           b_adapt_penaf =BLOC(condition = """(equal_to("ADAPTATION", 'ADAPT_COEF')) or (equal_to("ADAPTATION", 'TOUT'))""",
                                                                             GLIS_MAXI  =SIMP(statut='f',typ='R'),
                                                                             COEF_PENA_FROT  =SIMP(statut='f',typ='R',defaut=1.0E7),
                                                          ),
                                                           b_noadapt_penaf =BLOC(condition = """(equal_to("ADAPTATION", 'CYCLAGE')) or (equal_to("ADAPTATION", 'NON'))""",
