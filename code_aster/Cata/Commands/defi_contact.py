...
# --- Method for friction
                                          ...
                                          b_frot_pena=BLOC(condition = """(equal_to("ALGO_FROT", 'PENALISATION'))  """, fr=tr("Paramètres de la méthode pénalisée"),
                                                           ...
                                                           #b_adaptfr_penal =BLOC(condition = """(equal_to("ADAPTATION", 'ADAPT_COEF')) or (equal_to("ADAPTATION", 'TOUT'))""",
                                                                             #PENE_MAXI  =SIMP(statut='f',typ='R'),
                                                                             #COEF_PENA_CONT  =SIMP(statut='f',typ='R',defaut=100.0E+0),
                                                          #),
                                                           #b_noadaptfr_penal =BLOC(condition = """(equal_to("ADAPTATION", 'CYCLAGE')) or (equal_to("ADAPTATION", 'NON'))""",
                                                                             ##PENE_MAXI  =SIMP(statut='f',typ='R'),
                                                                             #COEF_PENA_CONT  =SIMP(statut='o',typ='R',),
                                                          #),
                                          ),
