...
from code_aster.libaster import GenericMechanicalLoad


def AFFE_CHAR_MECA(**curDict):
    returnLoad = GenericMechanicalLoad.create()
    cdef string name = returnLoad.getInstance().getName()
    cdef string type = returnLoad.getInstance().getType()
    syntax = CommandSyntax("AFFE_CHAR_MECA")

    syntax.setResult(name, type)

    syntax.define(curDict)
    cdef INTEGER numOp = 7
