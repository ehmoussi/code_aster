...
from code_aster.Modeling.CrackTip cimport CrackTip


def DEFI_FOND_FISS(**curDict):
    returnCrackDef = CrackTip()
    cdef string name = returnCrackDef.getInstance().getName()
    cdef string type = returnCrackDef.getInstance().getType()
    syntax = CommandSyntax("DEFI_FOND_FISS")

    syntax.setResult(name, type)

    syntax.define(curDict)
    cdef INTEGER numOp = 55
