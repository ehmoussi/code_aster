...
def FP(rho,cd,ca,D,xx,yy,zz,tps,H,T,L,THETA,UC,THETAC,PROINF,dd,LSTRECH,vite_x,vite_y,vite_z,dx,dy,dz,dir,cinelin,projvect,dragquad):
...
#       Direction
        if dir   == 1:
            F=FX + numpy.pi*D**2/4.*(1.+ca)*rho*atx
        elif dir == 2:
            F=FY + numpy.pi*D**2/4.*(1.+ca)*rho*aty
        elif dir == 3:
            F=FZ + numpy.pi*D**2/4.*(1.+ca)*rho*atz
        else:
            F=0
    else:
        F=0.
