...
ACCE1C=GENE_ACCE_SEISME(INFO = 2,
              INIT_ALEA = 100000, NB_POIN = 2**10,
              PAS_INST  = DT, PESANTEUR = 9.81, DUREE_PHASE_FORTE = 10.,COEF_CORR=0.4,
              DSP        = _F(AMOR_REDUIT  = amor, FREQ_FOND = frr,),
              MODULATION = _F(TYPE = "JENNINGS_HOUSNER", ECART_TYPE = ecart),),


FONCT1bX=RECU_FONCTION( TABLE=ACCE1C,
                         FILTRE=(_F(NOM_PARA = 'NUME_ORDRE', VALE_I = 1),
                                 _F(NOM_PARA = 'NOM_PARA', VALE_K = 'ACCE1'),),
                        NOM_PARA_TABL='FONCTION',)

FONCT1bY=RECU_FONCTION( TABLE=ACCE1C,
                         FILTRE=(_F(NOM_PARA = 'NUME_ORDRE', VALE_I = 1),
...
ini_alea = [4023595400, 3382282977, 1842436784, 3587755405, 970681910, 3378139375, 185071033, 3725816495, 3770724822, 3378549732, 1390738741, 2424415233, 1195348504, 2875986569, 2588451870, 1848740672, 1026195745, 3781147206, 2344155955, 1222934458, 2471949151, 430151656, 1449699319, 1363965239, 340307910, 1407568363, 569729398, 2428348222, 2384592973, 287703465, 283129926, 4213482666, 4243887880, 2567272725, 1260005109, 3344285233, 265688195, 3145972743, 3128872165, 3886430842, 1207112003, 4013302884, 1356959730, 1756159259, 4002791369, 3880588004, 963429748, 48800114, 2632831028, 3105129498]

for iii in range(1, Nsim+1):
   print("RUN  ", iii+1)
   VECT=GENE_ACCE_SEISME(INIT_ALEA = ini_alea[iii-1],
...
TEST_FONCTION(VALEUR=(_F(VALE_CALC= 1.485287768951E-04 ,
...
TEST_FONCTION(VALEUR=(_F(VALE_CALC= 3.861630564432E-03,
