...
champ_meca=CREA_CHAMP(TYPE_CHAM='NOEU_FORC_R', OPERATION='AFFE', MAILLAGE=pMesh2,
                      AFFE=_F(TOUT='OUI', NOM_CMP=('FX', 'FY'), VALE=(10000, 0),),
                      )

resu_meca=CREA_RESU(OPERATION='AFFE', TYPE_RESU='EVOL_CHAR',
                    NOM_CHAM='FVOL_2D',
                    AFFE=_F(CHAM_GD=champ_meca,
                            INST=0.0,),
                    )
...
# DX displacement on nodes "N1" and "N3", comparison with sequential results (PESANTEUR)
if rank == 0:
